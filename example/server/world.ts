import {
  ...
  KIND_TILE, KIND_TREE, KIND_ROCK, KIND_NPC,
} from '../shared.js';
import { spawnEntity, spawnTile, entityAt } from './entities.js';

// Server-side tile lookup (for pathfinding/walkability)
...
export function generateWorld() {
  ...
  // Spawn tile entities (synced to clients via ECS)
  for (let y = 0; y < WORLD_TILES; y++) {
    for (let x = 0; x < WORLD_TILES; x++) {
      spawnTile(x, y, tileMap[y * WORLD_TILES + x]);
    }
  }
  ...
  for (let i = 0; i < 80; i++) {
    ...
    if (tileAt(x, y) === TILE_GRASS && !entityAt(x, y, KIND_TREE)) {
  ...
  for (let i = 0; i < 40; i++) {
    const x = rng(WORLD_TILES), y = rng(WORLD_TILES);
    if (tileAt(x, y) === TILE_GRASS && !entityAt(x, y, KIND_ROCK)) {
