...
import { Position, EntityType, Health, Appearance, Owner, KIND_TILE } from '../shared.js';
...
export function spawnTile(x: number, y: number, tileType: number): EntityId {
...
export function spawnPlayer(x: number, y: number, clientId: number, variant: number): EntityId {
  const eid = em.createEntityWith(
    Position, { x, y },
    EntityType, { kind: 1 },
