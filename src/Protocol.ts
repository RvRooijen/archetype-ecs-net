...
import type { ClientDeltaMessage, DeltaMessage, FieldInfo, FullStateMessage, NetMessage, WireType } from './types.js';
import { MSG_CLIENT_DELTA, MSG_DELTA, MSG_FULL } from './types.js';
...
export class ProtocolDecoder {
  ...
  decode(buffer: ArrayBuffer, registry: ComponentRegistry): NetMessage {
    ...
    } else if (msgType === MSG_CLIENT_DELTA) {
      return this.decodeClientDelta(registry);
    }
  ...
  private decodeDelta(registry: ComponentRegistry): DeltaMessage {
    ...
    // Attached
    const attachedEntityCount = this.readU16();
    const attached: DeltaMessage['attached'] = [];
    for (let i = 0; i < attachedEntityCount; i++) {
      const netId = this.readVarint();
      const wireCount = this.readU8();
      for (let c = 0; c < wireCount; c++) {
        const wireId = this.readU8();
        const reg = registry.byWireId(wireId);
        if (!reg) throw new Error(`Unknown wire ID: ${wireId}`);
        attached.push({ netId, componentWireId: wireId, data: this.readComponentData(reg.fields) });
      }
    }

    // Detached
    const detachedEntityCount = this.readU16();
    const detached: DeltaMessage['detached'] = [];
    for (let i = 0; i < detachedEntityCount; i++) {
      const netId = this.readVarint();
      const wireCount = this.readU8();
      for (let c = 0; c < wireCount; c++) {
        const wireId = this.readU8();
        detached.push({ netId, componentWireId: wireId });
      }
    }

    return { type: MSG_DELTA, created, destroyed, updated, attached, detached };
  }

  private decodeClientDelta(registry: ComponentRegistry): ClientDeltaMessage {
    const updatedEntityCount = this.readU16();
    const updated: ClientDeltaMessage['updated'] = [];
    ...
    for (let i = 0; i < updatedEntityCount; i++) {
      ...
      for (let c = 0; c < compCount; c++) {
        ...
        const reg = registry.byWireId(wireId);
        if (!reg) throw new Error(`Unknown wire ID: ${wireId}`);
        if (!reg.clientOwned) throw new Error(`Wire ID ${wireId} (${reg.name}) is not clientOwned`);
    ...
    // Attached
    const attachedEntityCount = this.readU16();
    const attached: ClientDeltaMessage['attached'] = [];
    for (let i = 0; i < attachedEntityCount; i++) {
      const netId = this.readVarint();
      const wireCount = this.readU8();
      for (let c = 0; c < wireCount; c++) {
    ...
    // Detached
    const detachedEntityCount = this.readU16();
    const detached: ClientDeltaMessage['detached'] = [];
    for (let i = 0; i < detachedEntityCount; i++) {
      const netId = this.readVarint();
      const wireCount = this.readU8();
      for (let c = 0; c < wireCount; c++) {
    ...
    return { type: MSG_CLIENT_DELTA, updated, attached, detached };
  }
}
