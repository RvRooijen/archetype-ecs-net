...
export interface ClientDelta {
  ...
  /** NetIds with component attaches (known, not entering/leaving) */
  readonly attached: number[];
  /** NetIds with component detaches (known, not entering/leaving) */
...
export function createClientView(): ClientView {
  ...
  return {
    ...
    update(interest: ReadonlySet<number>, changeset: Changeset): ClientDelta {
      ...
      // Build fast enter set for isEnter checks
      const enterSet = new Set(enters);

      // 4. Dirty entities that are in known (and not just entered) → update
      for (const entry of changeset.dirty) {
        if (known.has(entry.netId) && !createdSet.has(entry.netId) && !enterSet.has(entry.netId)) {
          updates.push(entry.netId);
        }
      }

      // 5. Attached entities: known, not entering or leaving
      const leaveSet = new Set(leaves);
      for (const entry of changeset.attached) {
        if (known.has(entry.netId) && !enterSet.has(entry.netId) && !leaveSet.has(entry.netId)) {
          attached.push(entry.netId);
        }
      }

      // 6. Detached entities: known, not entering or leaving
      for (const entry of changeset.detached) {
        if (known.has(entry.netId) && !enterSet.has(entry.netId) && !leaveSet.has(entry.netId)) {
          detached.push(entry.netId);
        }
      }

      return { enters, leaves, updates, attached, detached };
    },
  };
}
