...
import { MSG_DELTA, MSG_CLIENT_ID, MSG_RECONNECT } from '../src/types.js';
...
function openWs(url: string): Promise<WebSocket> {
  return new Promise(r => {
    ...
    ws.on('open', () => { sendHandshake(ws); r(ws); });
  });
}
...
describe('tick(filter)', () => {
  function recv(ws: WebSocket): Promise<ArrayBuffer> {
    return new Promise(r => {
      const handler = (d: Buffer) => {
        ...
        if (new Uint8Array(buf)[0] === MSG_CLIENT_ID) { ws.once('message', handler); return; }
        r(buf);
      };
  ...
  it('broadcast tick() still works unchanged', async () => {
    ...
    await server.start();
    const ws = await openWs('ws://localhost:19931');
  ...
  it('3 clients with disjoint, overlapping, and shifting filters across multiple ticks', async () => {
    ...
    const ws1 = await openWs('ws://localhost:19932');
    const full1 = decoder.decode(await recv(ws1), registry) as FullStateMessage;
    // Wait for onConnect to fire
    await new Promise(r => setTimeout(r, 20));

    const ws2 = await openWs('ws://localhost:19932');
    const full2 = decoder.decode(await recv(ws2), registry) as FullStateMessage;
    await new Promise(r => setTimeout(r, 20));

    const ws3 = await openWs('ws://localhost:19932');
  ...
  it('deduplicates encoding for clients with identical deltas', async () => {
    ...
    const mockTransport = {
      async start(_port: number, handlers: any) {
        // Simulate 3 clients connecting with handshake
        for (let i = 1; i <= 3; i++) {
          handlers.onOpen(i);
          const hs = new ArrayBuffer(5);
          new DataView(hs).setUint8(0, MSG_RECONNECT);
          handlers.onMessage(i, hs);
        }
      },
      async stop() {},
